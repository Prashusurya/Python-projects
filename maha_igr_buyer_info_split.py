# -*- coding: utf-8 -*-
"""
Created on Wed Oct 16 18:49:04 2024
...
df=pd.read_sql('select Id,DocNo,Buyer from tbl_maharashtra_igr_basic',raw)

done=pd.read_sql("select Id from tbl_maha_buyer_info_correct", tempdb)
...
df1['Buyer']=df1['Buyer'].apply(lambda x:x.replace('\xa0', ' '))
df1['Buyer']=df1['Buyer'].apply(lambda x:x.replace(' - ', ''))
df1['Buyer']=df1['Buyer'].apply(lambda x:x.replace('\u200b', ' '))
df1['Buyer']=df1['Buyer'].apply(lambda x:x.replace('\u200d', ' '))
df1['Buyer']=df1['Buyer'].apply(lambda x:x.replace(': -', ':-'))

df1['Buyer_name']=df1['Buyer'].apply(lambda x:re.findall(r'नाव:\-(.*?)वय\:',x))
df1['Buyer_name']=df1['Buyer_name'].apply(lambda x:x[0] if len(x)>0 else None)
df1['Buyer_name']=df1['Buyer_name'].str.replace(' वय','').str.replace('नाव:-','')
df1['Buyer_name']=df1['Buyer_name'].str.strip()

df1=df1[~df1['Buyer_name'].isnull()]

df1['Buyer_name']=df1['Buyer_name'].apply(lambda x:x.split(' ') if isinstance(x,str) else x)
df1['Buyer_name']=df1['Buyer_name'].apply(lambda x:' '.join(a.replace(' ','') for a in x))

df1['Buyer_name1']=df1['Buyer_name'].apply(lambda x:re.findall(r'(मेसर्स|मे\..*?)तर्फे|स्वतः(.*?एन्टरप्राइस\s*)',x))

df1['Buyer_name1']=df1['Buyer_name1'].apply(lambda x: str(x))

df1['Buyer_name1']=df1['Buyer_name1'].str.replace('[','').str.replace(']','').str.replace(')','').str.replace('(','').str.replace("'",'')

df1['Buyer_name1']=df1['Buyer_name1'].apply(lambda x: x.split(', ')[0])

df1['Buyer_name1']=df1['Buyer_name1'].str.strip()
df1['Buyer_name1']=df1['Buyer_name1'].apply(lambda x:x.replace('तर्फे', ''))
df1['Buyer_name1']=df1['Buyer_name1'].apply(lambda x:x.replace('मे.', 'मेसर्स '))
df1['Buyer_name1']=df1['Buyer_name1'].apply(lambda x:x.replace('में.', 'मेसर्स '))
df1['Buyer_name1']=df1['Buyer_name1'].apply(lambda x:x.replace('में ', 'मेसर्स '))
df1['Buyer_name1']=df1['Buyer_name1'].apply(lambda x:x.replace('मे ', 'मेसर्स '))
df1['Buyer_name1']=df1['Buyer_name1'].apply(lambda x:x.replace('-', ''))
df1['Buyer_name1']=df1['Buyer_name1'].apply(lambda x:x.replace('--', ''))
df1['Buyer_name1']=df1['Buyer_name1'].str.strip()

df1['Buyer_name1']=df1['Buyer_name1'].apply(lambda x: None if x=='मेसर्स' else x)
df1['Buyer_name1']=df1['Buyer_name1'].apply(lambda x: None if isinstance(x, str) and len(x)==0 else x)
final=df1[~df1['Buyer_name1'].isnull()]
df1=df1[df1['Buyer_name1'].isnull()]
...
for pattern in patterns:
    ...
    for name in df2['Buyer_name']:
        ...
        if match:
            ...
            matched.append(None) 
            found_pattern.append(None)
    df2['Buyer_name1']=matched
    df2['Pattern']=found_pattern
    found_matches=df2[~df2['Buyer_name1'].isnull()]
    final1=pd.concat([final1,found_matches], axis=0)
    df2=df2[df2['Buyer_name1'].isnull()]
    
final1['Buyer_name1']=final1['Buyer_name1'].apply(lambda x: x.split('तर्फे')[-1])
...
final1['Buyer_name1']=final1['Buyer_name1'].apply(lambda x:''.join(x.split('2)')[0]))
final1['Buyer_name1']=final1['Buyer_name1'].apply(lambda x: x.split('म्हणून')[-1])
final1['Buyer_name1']=final1['Buyer_name1'].apply(lambda x: x.split(',2)')[0])
final1['Buyer_name1']=final1['Buyer_name1'].apply(lambda x: x.split(', 2)')[0])
final1['Buyer_name1']=final1['Buyer_name1'].apply(lambda x: x.split(',2.')[0])
final1['Buyer_name1']=final1['Buyer_name1'].apply(lambda x: x.split(', 2.')[0])
final1['Buyer_name1']=final1['Buyer_name1'].apply(lambda x: x.split(' 2)')[0])
final1['Buyer_name1']=final1['Buyer_name1'].apply(lambda x: x.split('२)')[0])
final1['Buyer_name1']=final1['Buyer_name1'].apply(lambda x:''.join(x.split('२')[0]))
final1['Buyer_name1']=final1['Buyer_name1'].str.strip()

final=pd.concat([final,final1], axis=0, ignore_index=True)

df2['Buyer_name1']=df2['Buyer_name'].apply(lambda x: x.split('तर्फे')[0] if isinstance(x, str) else x)

final=pd.concat([final,df2], axis=0, ignore_index=True)
final=final[~final['Buyer_name1'].isnull()]
# final=final.drop('Pattern', axis=1)
final=final.rename(columns={'Buyer_name1':'first_Buyer'})
df=final.drop_duplicates(subset='Id')

df['first_Buyer']=df['first_Buyer'].replace('\xa0', ' ')
df['first_Buyer']=df['first_Buyer'].replace('\u200d', ' ')
df['first_Buyer']=df['first_Buyer'].replace('\u200c',' ')
df['first_Buyer']=df['first_Buyer'].replace('-', ' ')
df['first_Buyer']=df['first_Buyer'].str.strip()
...
df['Buyer_age']=df['Buyer'].apply(lambda x: find_patterns(age_patterns,x))
df['Buyer_age']=df['Buyer_age'].str.replace('\xa0', ' ')
df['Buyer_age']=df['Buyer_age'].str.strip()
df['Buyer_age']=df['Buyer_age'].str.extract(r'([-+]?\d*\.?\d+)')

address_patterns=[r'पत्ता\:\-\s?(.*?)पिन\s?कोड']

df['Buyer_address']=df['Buyer'].apply(lambda x: find_patterns(address_patterns,x))
df['Buyer_address']=df['Buyer_address'].str.replace('\xa0', ' ')
df['Buyer_address']=df['Buyer_address'].str.strip()

pincode_patterns=[r'पिन\s?कोड\:?\-?\s?([0-9]{6})']

df['Buyer_pincode']=df['Buyer'].apply(lambda x: find_patterns(pincode_patterns,x))
df['Buyer_pincode']=df['Buyer_pincode'].str.replace('\xa0', ' ')
df['Buyer_pincode']=df['Buyer_pincode'].str.strip()

pan_patterns=[r'([A-Z]{5}[0-9]{4}[A-Z])']

df['Buyer_pan_number']=df['Buyer'].apply(lambda x: find_patterns(pan_patterns,x))
df['Buyer_pan_number']=df['Buyer_pan_number'].str.replace('\xa0', ' ')
df['Buyer_pan_number']=df['Buyer_pan_number'].str.strip()
...
df['Buyer_type']=df['Buyer_pan_number'].apply(lambda x: Buyer_type(x))
df['Buyer_name']=df['Buyer_name'].str.replace(' - ',' ')
...
individual=df[df['Buyer_type'].str.contains('Individual',na=True)]

changed=[]
for i in individual['Buyer_name']:
    if any(word in i for word in patterns):
        changed.append('Company')
    else:
        changed.append('Individual')
        
individual['Buyer_type']=changed

df=df[~df['Buyer_type'].str.contains('Individual',na=True)]
...
df['Buyer_age']=df['Buyer_age'].apply(lambda x:'0' if x==None else x)

df=df.map(lambda x: None if isinstance(x, str) and x=='None' else x)
df=df.map(lambda x: None if isinstance(x, str) and x=='' else x)
df=df[~df['first_Buyer'].isnull()]

df['Id']=df['Id'].apply(lambda x: int(x))

df=df.drop_duplicates(subset='Id')

df.to_excel('buyer_new_split_check_14112024.xlsx',index=False)

# df.to_sql('tbl_maha_buyer_info_correct', con=engine2, if_exists='append', index=False)
...
#     path=r'\\192.168.0.28\datascience\Marathi Translate\Marathi\buyer info\\'
